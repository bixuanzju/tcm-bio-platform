...
public class BioInferDaoImpl extends JdbcDaoSupport implements BioInferDao {
	...
	private static final Logger LOGGER = LogManager
			.getLogger(BioInferDaoImpl.class.getName());

	@Override
	public ArrayList<BioInferData> getDrugInference(final String bio,
			final Integer start, final Integer offset) {

		String sparql0 = "sparql select * where {"
				+ "graph<http://linkedlifedata.com/resource/drugbank> {?s ?p \"" + bio
				+ "\"}}";

		LOGGER.debug("get drug name1 - query virtuoso: {}", sparql0);

		List<Map<String, Object>> rows0 = getJdbcTemplate().queryForList(sparql0);

		if (rows0.size() == 0) {

			sparql0 = "sparql select distinct ?drugName where {graph<http://linkedlifedata.com/resource/drugbank>"
					+ "{?s ?p ?drugName ."
					+ "filter (?drugName LIKE \"@"
					+ bio
					+ "\")}} ";
			LOGGER.debug("get drug name2 - query virtuoso: {}", sparql0);
		...
		for (final Map<String, Object> row0 : rows0) {
			...
			LOGGER.debug("get drug inference result - query virtuoso: {}", sparql);
			...
			try {
				...
				for (final Map<String, Object> map : rows) {
					...
					bioInferData.setDrugName(drugName);
	...
	@Override
	public ArrayList<BioInferData> getGeneNameInference(final String geneName,
			final Integer start, final Integer offset) {
	...
	public ArrayList<BioInferData> getPAInference(final String PAName,
		...
		final String sparql = "sparql select * where {"
				+ "graph<http://linkedlifedata.com/resource/drugbank> {?targetID drugbank:swissprotId <"
				+ PAName
				+ "> } . "
				+ "graph<http://linkedlifedata.com/resource/drugbank> {?drugID drugbank:target ?targetID} . "
				+ "graph<http://linkedlifedata.com/resource/drugbank> {?drugID rdfs:label ?drugName} . "
	...
	@Override
	public ArrayList<BioInferData> getGOIDInference(final String GOID,
			final Integer start, final Integer offset) {

		final String sparql = "sparql select * where {"
				+ "graph<http://uniprot/protein_gene_mapping> {?proteinAcce uniprotGO:classifiedWith <"
				+ GOID
				+ "> } . "
				+ "graph<http://linkedlifedata.com/resource/drugbank> {?targetID drugbank:swissprotId ?proteinAcce} . "
				+ "graph<http://linkedlifedata.com/resource/drugbank> {?drugID drugbank:target ?targetID} . "
	...
	@Override
	public ArrayList<BioInferData> getDisInference(final String disName,
			final Integer start, final Integer offset) {
		
		String sparql0 = "sparql select * where {"
				+ "graph<http://localhost:8890/TCMGeneDIT> {?tcmName TCMGeneDIT:treatment <"
				+ disName + ">}} " + "limit(" + offset + ") offset(" + start + ")";

		LOGGER.debug("get disease name1 - query virtuoso: {}", sparql0);

		List<Map<String, Object>> rows = getJdbcTemplate().queryForList(sparql0);

		if (rows.size() == 0) {

			sparql0 = "sparql select * where {graph<http://localhost:8890/TCMGeneDIT>"
					+ "{?tcmName TCMGeneDIT:treatment ?disName ."
					+ "filter (?disName LIKE \"@"
					+ disName
					+ "\")}} limit(" + offset + ") offset(" + start + ")";
			
			LOGGER.debug("get dis name2 - query virtuoso: {}", sparql0);
			rows = getJdbcTemplate().queryForList(sparql0);
			...
			try {
				...
				for (final Map<String, Object> map : rows) {
					final BioInferData bioInferData = new BioInferData();
					bioInferData.setTcmName(map.get("tcmName").toString());
					bioInferData.setDiseaseName(map.get("disName").toString());
					bioInferDatas.add(bioInferData);
				}
				return bioInferDatas;
			}
	...
	@Override
	public ArrayList<String> getDisID2DisName(final String disid,
			final Integer start, final Integer offset) {

		final String sparql = "sparql select * where {"
				+ "graph<http://localhost:8890/tcm_diseasesome_mapping> {?diseaseName owl:sameAs "
				+ disid + " }}";
	...
	@Override
	public ArrayList<String> getDisName2TCMName(final String disname,
			final Integer start, final Integer offset) {
	...
	@Override
	public ArrayList<String> getDrugID2DisID(final String drugID,
			final Integer start, final Integer offset) {

		final String sparql = "sparql select * where {"
				+ "graph<http://linkedlifedata.com/resource/diseasome> {?diseaseID diseasesome:possibleDrug "
				+ drugID + "}} ";
	...
	@Override
	public ArrayList<String> getDrugName2DrugID(final String drugName,
			final Integer start, final Integer offset) {
	...
	@Override
	public ArrayList<String> getPA2TargetName(final String PAname,
			final Integer start, final Integer offset) {
	...
	@Override
	public ArrayList<String> getTargetName2DrugName(final String targetName,
			final Integer start, final Integer offset) {

		final String sparql = "sparql select * where {"
				+ "graph<http://linkedlifedata.com/resource/drugbank> {?targetID drugbank:name \""
				+ targetName
				+ "\" } . "
				+ "graph<http://linkedlifedata.com/resource/drugbank> {?drugID drugbank:target ?targetID} . "
				+ "graph<http://linkedlifedata.com/resource/drugbank> {?drugID rdfs:label ?drugName} }";
	...
	@Override
	public ArrayList<String> getDrugName2DisName(final String drugName,
			final Integer start, final Integer offset) {
	...
	@Override
	public ArrayList<String> getGeneName2PA(final String geneName,
			final Integer start, final Integer offset) {
	...
	@Override
	public ArrayList<String> getGO2PA(final String GOName, final Integer start,
	...
	public ArrayList<String> getGeneID(final String protein, final Integer start,
		...
		final String sparql = "sparql select * from <" + Protein_Gene_Mapping
				+ "> where {" + protein + " <" + UniprotGO_ClassifiedWith
				+ "> ?geneID}";
	...
	@Override
	public ArrayList<String> getGeneProduct(final String geneID,
			final Integer start, final Integer offset) {

		final String sparql = "sparql select * from <" + Gene_Ontology
				+ "> where {" + geneID + " " + Rdfs_Label + " ?geneProduct}";
